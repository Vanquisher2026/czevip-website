 // Cloudflare Pages Function - influencer / creator application
 export async function onRequestPost({ request, env }) {
   ...
   if (!body.email || !body.handle) return json({ error: 'Missing fields' }, 400, cors);
   const record = {
     kind: 'partner',
   ...
   await notify(env, 'Creator app: ' + record.name + ' - ' + (body.platform || '') + ' ' + (body.handle || ''), record);
