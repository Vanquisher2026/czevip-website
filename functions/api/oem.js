 // Cloudflare Pages Function - OEM / manufacturing inquiry
 export async function onRequestPost({ request, env }) {
   ...
   if (!body.email || !body.name) return json({ error: 'Missing fields' }, 400, cors);
   const record = {
     kind: 'oem',
   ...
   await notify(env, 'OEM inquiry: ' + record.name + ' (' + (body.company || '') + ')', record);
